...
command_string = Format('create table %s(id serial, description text, num_before int, num_after int)',sf_table_meta_res); 
execute command_string;
...
command_string = Format('insert into %s(description, num_before, num_after) VALUES(%L,%s,%s)',sf_table_meta_res,
'Polygons with attribute values ('||sf_table_in ||'->'||sf_table_out||')',num_before,num_after);
execute command_string;
...
RAISE NOTICE 'select * from  %', sf_table_meta_res;
...
('test_topo_t2',0.00001,false,null), -- TYPE resolve_overlap_data_topology30,3000)
...
select * from  test_topo_t2.simple_compare_result;
