...
BEGIN
  ...
  data_type := (
    ...
    WHERE b.relname = _table
      AND c.nspname = _schema
      AND a.attname = _c);
  RETURN data_type;
END
$body$
...
DROP FUNCTION IF EXISTS topo_update.view_split_distinct_func (input_table_name varchar, input_table_geo_column_name varchar, bb geometry, inside_cell_data boolean);
 ...
 --     AND  (gt.border_lines is null OR ST_intersects(gt.border_lines,lg.geo) = false)
  ...
  FROM (
    SELECT '{"type": "Feature",' || '"geometry":' || ST_AsGeoJSON (lg3.geo, 10, 2)::json || ',' || '"properties":' || Row_to_json((
        SELECT l FROM (
            SELECT "oppdateringsdato") AS l)) || '}' AS json, lg3.geo, lg3.objectid
    FROM ( SELECT DISTINCT lg2.geo, Now() AS "oppdateringsdato", 1 AS "objectid"
      FROM (
        SELECT (ST_Dump (ST_LineMerge (ST_Union (lg.geo)))).geom AS geo
        FROM tmp_data_border_lines lg) lg2) AS lg3) AS f;
END
$function$;
