    ...
    BEGIN
      ...
      WHILE i < dim LOOP
        ...
        BEGIN
          ...
          BEGIN
	        -- find cloeste points
            tmp_egde_geom := topo_update.extend_line (new_egde_geom, _snap_tolerance*4);
            ...
            -- 2
            BEGIN
              tmp_egde_geom := ST_ExteriorRing (ST_buffer (new_egde_geom, _snap_tolerance * 5, 1));
              single_line_geo = topo_update.get_single_lineparts (tmp_egde_geom);
              SELECT ST_NumGeometries (single_line_geo) INTO dim2;
              WHILE i2 < dim2 LOOP
                i2 := i2 + 1;
                tmp_egde_geom := ST_GeometryN (single_line_geo, i2);
                ---
                BEGIN
                  command_string := Format('select topology.TopoGeo_addLinestring(%s,%L,%s)', Quote_literal(_topology_name), tmp_egde_geom, _snap_tolerance * 2);
                  EXECUTE command_string;
                  EXCEPTION
                  WHEN OTHERS THEN
