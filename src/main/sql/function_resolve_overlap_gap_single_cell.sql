...
EXECUTE Format('ALTER TABLE %s ADD column %s serial',temp_table_name, temp_table_id_column);
     ...
     -- Update cl
		  ...
		  ',','r.',',',temp_table_name,'{}',temp_table_id_column,input_table_geo_column_name,'_other_intersect_id_list');
...
-- Drop/Create a temp to hold data temporay for job
  ...
  IF used_time > 10 THEN
    ...
    EXECUTE FORMAT('INSERT INTO %s (execute_time, info, sql, geo) VALUES (%s, %L, %L, %L)',
